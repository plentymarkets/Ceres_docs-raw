...
The afterKey is only set when the variation selection requires more data than is already loaded in the client.|link:http://php.net/array[`array`^]
...
a|`$assetName`
|Key for the assets to be loaded.|link:http://php.net/int[`int`^]
...
a|`$setComponents`
|Some data of the variations contained as a component for the current item if the item is of the type "set".|         xref:5.0.0@plugin-::.adoc#[``]
a|`$setAttributeMap`
